...
class ExercisesDataBaseHelper: Repository {
    ...
    private let routineId = Expression<Int64>("RoutineID")
    ...
    private let loadValue = Expression<Double>("LoadValue")
    private let loadUnit = Expression<String>("LoadUnit")
    ...
    private let intensityRangeFrom = Expression<Int>("RPERangeFrom")
    private let intensityRangeTo = Expression<Int>("RPERangeTo")
    private let intensityIndex = Expression<String>("IntensityIndex")
    private let pace = Expression<String>("Pace")
    
    private let routinesTable = Table(RoutinesDataBaseHelper.TABLE_NAME)
    private let routinesTableRoutineId = Expression<Int64>(RoutinesDataBaseHelper.ROUTINE_ID_COLUMN)
    private let routinesTablePlanId = Expression<Int64>(RoutinesDataBaseHelper.PLAN_ID_COLUMN)
    private let routinesTableRoutineName = Expression<String>(RoutinesDataBaseHelper.ROUTINE_NAME_COLUMN)
